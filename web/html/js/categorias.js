...
document.addEventListener('DOMContentLoaded', function() {
    const contenedorPeliculas = document.querySelectorAll(".div-pelicula");

    contenedorPeliculas.forEach((pelicula) => {
        const contenedorImagen = pelicula.querySelector(".div-cartel");
        const nombrePeli = pelicula.querySelector(".nombrePelicula").textContent;

        contenedorImagen.addEventListener('click', function(event) {
            const urlPelicula = "Pelicula/" + encodeURIComponent(nombrePeli);
